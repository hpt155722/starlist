...
function signup() {
    ...
    if (userGivenEmail == "" || createdUsername == "" || createdPassword == "" || confirmPassword == "" ) {
        ...
        $('.errorMessage').show();
    } else if (!isValidEmail(userGivenEmail)) { //Validate if email is in correct format
        $('.errorMessage').text('Not a valid email. Please try again.');
        $('.errorMessage').show();
    }  else if (createdPassword != confirmPassword) { //Validate if password is confirmed
        $('.errorMessage').text('Passwords do not match. Please try again.');
        $('.errorMessage').show();
    } else if (createdPassword.length < 8) { //Validate if password is long enough
